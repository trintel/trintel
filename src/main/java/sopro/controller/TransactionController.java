...
public class TransactionController {
    ...
    public String listTransactions(Model model, @AuthenticationPrincipal User user, @RequestParam(defaultValue = "true") boolean sortByNewest, @RequestParam(defaultValue = "") String buyer, @RequestParam(defaultValue = "") String seller) {
        List<Transaction> transactions = new ArrayList<>();
        if (user.getRole().equals("ADMIN")) { // Admins can see all transactions
            transactions = transactionRepository.findByBuyerAndSeller(buyer, seller);
            if(sortByNewest) {
